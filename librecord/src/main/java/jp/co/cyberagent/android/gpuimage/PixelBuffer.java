...
//        mRenderer.onDrawFrame(mGL);
        mRenderer.onDrawFrame(mGL);
        if(null == outputByteBuffer)
        {
        	outputByteBuffer = ByteBuffer.allocate(mWidth * mHeight * 4); // 分配输出数据内存
        }
        
        mGL.glReadPixels(0, 0, mWidth, mHeight, GL_RGBA, GL_UNSIGNED_BYTE, outputByteBuffer); // 重复利用buffer
